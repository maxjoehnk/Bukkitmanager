...
public class RemoteServer extends Thread {
    ...
    public void run() {
        try {
            ...
            io.sendConsole("Remote Server started! Listening on Port: "
                    + sSocket.getLocalPort());
            while (listening)
                new RemoteConnection(sSocket.accept()).start();
        } catch (BindException e) {
            io.sendConsoleError("Failed to Bind to Port "
                    + config.getInt("RemoteServer.Port")
                    + ". Maybe another service is using it?");
