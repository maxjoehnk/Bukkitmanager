...
public class Permissions {
    ...
    // private static de.bananaco.bpermissions.imp.Permissions bPermissions =
    // null;
            ...
            if (Bukkit.getServer().getPluginManager().getPlugin("Vault") != null) {
                if (config.getBoolean("General.Permissions.Use-Vault")) {
                    ...
                    if (permissionProvider != null) {
                        ...
                        permSystem = "Vault";
                        io.sendConsole(io.translate("Permissions.Found")
                                .replaceAll("%perms%", "Vault"));
                    }
                }
            } else if (Bukkit.getServer().getPluginManager()
                    .getPlugin("PermissionsBukkit") != null) {
                permSystem = "PermissionsBukkit";
                io.sendConsole(io.translate("Permissions.Found").replaceAll(
                        "%perms%", "PermissionsBukkit"));
            } else if (Bukkit.getServer().getPluginManager()
                    .getPlugin("GroupManager") != null) {
                permSystem = "GroupManager";
                groupManager = ((GroupManager) Bukkit.getServer()
                ...
                // bPermissions = (de.bananaco.bpermissions.imp.Permissions)
                // Bukkit.getServer().getPluginManager().getPlugin("bPermissions");
                permSystem = "bPermissions";
                io.sendConsole(io.translate("Permissions.Found").replaceAll(
                        "%perms%", "bPermissions"));
            } else if (Bukkit.getServer().getPluginManager()
                    .getPlugin("zPermissions") != null) {
                permSystem = "zPermissions";
                io.sendConsole(io.translate("Permissions.Found").replaceAll(
                        "%perms%", "zPermissions"));
            } else if (Bukkit.getServer().getPluginManager()
                    .getPlugin("DroxPerms") != null) {
    ...
    public static boolean has(CommandSender sender, String perm, String cmd) {
        return has(sender, perm, cmd,
                config.getBoolean("General.Permissions.Log"));
    }
    ...
    public static boolean has(CommandSender sender, String perm, String cmd,
        ...
        if (!usePerms) hasPerm = sender.isOp();
            ...
            else if (permSystem.equalsIgnoreCase("Vault")) hasPerm = vault.has(
                    sender, perm);
            else if (permSystem.equalsIgnoreCase("GroupManager")) hasPerm = groupManager
                    .getWorldsHolder().getWorldPermissions(((Player) sender))
                    .has((Player) sender, perm);
            else if (permSystem.equalsIgnoreCase("PermissionsBukkit")) hasPerm = sender
                    .hasPermission(perm);
            else if (permSystem.equalsIgnoreCase("PermissionsEx")) hasPerm = pex
                    .has((Player) sender, perm);
            else if (permSystem.equalsIgnoreCase("bPermissions")) hasPerm = de.bananaco.bpermissions.imp.Permissions
                    .hasPermission((Player) sender, perm);
            else if (permSystem.equalsIgnoreCase("zPermissions")) hasPerm = sender
                    .hasPermission(perm);
            else if (permSystem.equalsIgnoreCase("DroxPerms")) hasPerm = sender
                    .hasPermission(perm);
        }
        if (hasPerm == false) {
            io.send(sender, io.translate("Command.NoPerm.Player"));
            if (log) io.sendConsoleWarning(io
                    .translate("Command.NoPerm.Console")
                    .replaceAll("%cmd%", cmd)
