        ...
        switch (type) {
            case AUTOBACKUP:
                if (backupThread == null || !backupThread.isAlive()
                        || !backupThread.getRun()) return false;
                else return true;
            case AUTOSAVE:
                if (saveThread == null || !saveThread.isAlive()
                        || !saveThread.getRun()) return false;
                else return true;
            case AUTOMESSAGE:
                if (msgThread == null || !msgThread.isAlive()
                        || !msgThread.getRun()) return false;
                else return true;
            default:
                return false;
        }
        ...
        switch (type) {
            case AUTOBACKUP:
                if (backupThread == null || !backupThread.isAlive()) {
                    ...
                    io.sendConsole(io
                            ...
                            .replaceAll("%thread%", "AutobackupThread")
                            .replaceAll("%interval%",
                                    config.getString("Autobackup.Interval")));
                }
                return true;
            case AUTOSAVE:
                if (saveThread == null || !saveThread.isAlive()) {
                    ...
                    io.sendConsole(io
                            .translate("Thread.Start")
                            .replaceAll("%thread%", "AutosaveThread")
                            .replaceAll("%interval%",
            ...
            case AUTOMESSAGE:
                if (msgThread == null || !msgThread.isAlive()) {
                    ...
                    io.sendConsole(io
                            .translate("Thread.Start")
                            .replaceAll("%thread%", "AutomessageThread")
                            .replaceAll("%interval%",
            ...
            case AUTOSAVE:
                if (saveThread == null) {
                    ...
                    if (saveThread.getRun()) {
                        io.sendConsole(io.translate("Thread.Stopping")
                                .replaceAll("%thread%", "AutosaveThread"));
                        saveThread.setRun(false);
                        try {
                            saveThread.join(5000);
                            saveThread = null;
                            io.sendConsole(io.translate("Thread.Stop")
                                    .replaceAll("%thread%", "AutosaveThread"));
                            return true;
                        } catch (InterruptedException e) {
                            io.sendConsoleWarning(io.translate("Thread.Error")
                                    .replaceAll("%thread%", "AutosaveThread"));
                ...
                if (msgThread == null) {
                    ...
                    if (msgThread.getRun()) {
                        ...
                        try {
                            ...
                            msgThread = null;
                            io.sendConsole(io
                                    .translate("Thread.Stop")
                                    .replaceAll("%thread%", "AutomessageThread"));
                            return true;
                        } catch (InterruptedException e) {
                            io.sendConsoleWarning(io
                                    .translate("Thread.Error")
                                    .replaceAll("%thread%", "AutomessageThread"));
                            if (config.getDebug()) e.printStackTrace();
                            return false;
                        }
