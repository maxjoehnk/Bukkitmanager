...
public class Bukkitmanager extends JavaPlugin {
    ...
    public void onDisable() {
        ...
        try {
            FileUtils.cleanDirectory(new File(getDataFolder().getParentFile()
                    .getAbsoluteFile().getParentFile()
                    + File.separator + "backups" + File.separator + "temp"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void onEnable() {
        ...
        io.init();
        if (config.getDebug()) io.sendConsoleWarning(io
                .translate("Plugin.Debug"));
        ...
        try {
            ...
            for (BukkitmanagerAddon addon : addons)
                addonStatistics.addSlice(addon, addon.getAddonName());
        } catch (IOException e) {
            e.printStackTrace();
        }
        new Permissions().init();
        if (config.getBoolean("Autobackup.Enabled")) ThreadManager
                .startThread(ThreadType.AUTOBACKUP);
        if (config.getBoolean("Autosave.Enabled")) ThreadManager
                .startThread(ThreadType.AUTOSAVE);
        if (config.getBoolean("Automessage.Enabled")) ThreadManager
                .startThread(ThreadType.AUTOMESSAGE);
        ...
        new Thread() {
            public void run() {
                ...
                if (config.getBoolean("General.Auto-Updater")
                        & !(config.getBoolean("PluginUpdater.Enabled") && config
                                .getBoolean("PluginUpdater.CheckOnStart"))) {
                    io.sendConsole(io
                    ...
                    if (!filePage.getName().equals(
                            "Bukkitmanager Beta "
                                    + getDescription().getVersion())) {
                        io.sendConsole(io.translate("AutoUpdater.NewVersion")
                                .replaceAll("%name%", filePage.getName()));
                        io.sendConsole(io
                                .translate("AutoUpdater.Running")
                                .replaceAll(
                                        "%name%",
                                        "Bukkitmanager Beta "
                                                + getDescription().getVersion()));
    ...
    public static void addCommandTracker(Command command) {
        if (commandStatistics == null) commandStatistics = new DonutGraph<CommandCategory>(
                "Command Usage");
        commandStatistics.addSlice(command.getCategory(), command.getCategory()
                .toString().toLowerCase(), command.getLabel().toLowerCase());
    ...
    public static void incrementCommandTracker(Command command) {
        commandStatistics.increment(command.getCategory(), command.getLabel()
                .toLowerCase());
