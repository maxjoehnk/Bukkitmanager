...
public class ServerLoadCmd extends Command {
    ...
    public ServerLoadCmd() {
        super("load", "Server.Load", "bm.server.load", new ArrayList<String>(),
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io.sendFewArgs(sender, "/bm server info");
            ...
            if (has(sender, "bm.server.load")) {
                int mb = 1024 * 1024;
                long maxMem = Runtime.getRuntime().maxMemory() / mb, totalMem = Runtime
                        .getRuntime().totalMemory() / mb, freeMem = Runtime
                        .getRuntime().freeMemory() / mb, usedMem = totalMem
                        - freeMem;
                String cpuUsage = "";
                if (config.getBoolean("General.CPULoad")) cpuUsage = String
