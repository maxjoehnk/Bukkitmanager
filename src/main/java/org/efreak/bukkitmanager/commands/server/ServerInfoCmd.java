...
public class ServerInfoCmd extends Command {
    ...
    public ServerInfoCmd() {
        super("info", "Server.Info", "bm.server.info", new ArrayList<String>(),
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io.sendFewArgs(sender, "/bm server info");
            ...
            if (has(sender, "bm.server.info")) {
                Long uptimeMilli = ManagementFactory.getRuntimeMXBean()
                        .getUptime();
                        ...
                        TimeUnit.MILLISECONDS.toSeconds(uptimeMilli)
                                - TimeUnit.MINUTES
                                        .toSeconds(TimeUnit.MILLISECONDS
                                                .toMinutes(uptimeMilli)));
                String cpuUsage = "";
                if (config.getBoolean("General.CPULoad")
                        && has(sender, "bm.server.load")) cpuUsage = String
                        .format("%.2f", CpuUsage.getUsage()) + "%";
                ...
                io.send(sender,
                        "JVM Name:            "
                                + System.getProperty("java.vm.name"), false);
                io.send(sender,
                        "JVM Version:         "
                                + System.getProperty("java.vm.version"), false);
                io.send(sender,
                        "Java Version:        "
                                + System.getProperty("java.version"), false);
                io.send(sender, "Uptime:              " + uptime, false);
                io.send(sender, "CPU Count:           "
                        + Runtime.getRuntime().availableProcessors(), false);
                if (has(sender, "bm.server.load")) {
                    int mb = 1024 * 1024;
                    long usedMem = (Runtime.getRuntime().totalMemory() - Runtime
                            .getRuntime().freeMemory()) / mb;
                    if (config.getBoolean("General.CPULoad")) io.send(sender,
                            "CPU Usage:           " + cpuUsage, false);
                    io.send(sender, "Used Memory:         " + usedMem + "MB",
                            false);
                }
            }
        }
