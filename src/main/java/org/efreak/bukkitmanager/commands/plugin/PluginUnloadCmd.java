...
public class PluginUnloadCmd extends Command {
    ...
    public PluginUnloadCmd() {
        super("reload", "Plugin.Reload", "bm.plugin.unload", Arrays
        ...
        if (args.length < 1) io.sendFewArgs(sender,
            ...
            if (has(sender, "bm.plugin.unload")) {
                if (args.length == 1) {
                    ...
                    io.send(sender,
                            io.translate("Command.Plugin.Unload.Success")
                                    .replaceAll("%plugin%", "Bukkitmanager"));
                } else {
                    if (args[1].equalsIgnoreCase("all")) {
                        PluginManager.unloadPlugins();
                        io.send(sender, io
                                .translate("Command.Plugin.Reload.Success.All"));
                        ...
                        PluginManager.unloadPlugin(PluginManager
                                .getPlugin(args[1]));
                        io.send(sender,
                                io.translate("Command.Plugin.Reload.Success")
