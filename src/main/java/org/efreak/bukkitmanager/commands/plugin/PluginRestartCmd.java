...
public class PluginRestartCmd extends Command {
    ...
    public PluginRestartCmd() {
        super("restart", "Plugin.Restart", "bm.plugin.restart", Arrays
        ...
        if (args.length < 1) io.sendFewArgs(sender,
            ...
            if (has(sender, "bm.plugin.restart")) {
                if (args.length == 1) {
                    ...
                    io.send(sender,
                            io.translate("Command.Plugin.Restart.Success")
                                    .replaceAll("%plugin%", "Bukkitmanager"));
                } else {
                    if (args[1].equalsIgnoreCase("all")) {
                        PluginManager.restartPlugins();
                        io.send(sender,
                                io.translate("Command.Plugin.Restart.Success.All"));
                        ...
                        if (args[1] == "Spout") io.sendWarning(sender,
                                io.translate("Command.Plugin.Restart.Spout"));
                        else {
                            PluginManager.restartPlugin(PluginManager
                                    .getPlugin(args[1]));
                            io.send(sender,
                                    io.translate(
                                            "Command.Plugin.Restart.Success")
