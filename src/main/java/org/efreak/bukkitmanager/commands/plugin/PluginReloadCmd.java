...
public class PluginReloadCmd extends Command {
    ...
    public PluginReloadCmd() {
        super("reload", "Plugin.Reload", "bm.plugin.reload", Arrays
                ...
                if (args.length == 1) {
                    ...
                    if (args[1].equalsIgnoreCase("all")) {
                        try {
                            ...
                            io.send(sender,
                                    io.translate("Command.Plugin.Reload.Success.All"));
                        } catch (UnknownDependencyException e) {
                            io.send(sender,
                        ...
                        try {
                            PluginManager.reloadPlugin(PluginManager
                                    .getPlugin(args[1]));
                            io.send(sender,
                                    io.translate(
                                            "Command.Plugin.Reload.Success")
