...
public class PluginUpdateCmd extends Command {
    ...
    public PluginUpdateCmd() {
        super("update", "Plugin.Update", "bm.plugin.update", Arrays
        ...
        if (args.length < 1) io.sendFewArgs(sender,
            ...
            if (args.length == 1) {
                ...
                if (plugin != null) {
                    ...
                    if (uptodate) io.send(
                            sender,
                            io.translate("PluginUpdater.UpToDate").replaceAll(
                                    "%plugin%", plugin.getName()));
                    else {
                        io.send(sender,
                                io.translate("PluginUpdater.NotUpToDate")
                                        .replaceAll("%plugin%",
                                                plugin.getName()));
                        io.createConversation(sender, "PluginUpdaterRequest",
                                new UpdatePluginPrompt(plugin.getName()));
                    }
                }
            }
        }
