...
public class AutomessageAddCmd extends Command {
    ...
    public AutomessageAddCmd() {
        super("add", "Automessage.Add", "bm.automessage.add", Arrays
                .asList("(msg)"), CommandCategory.AUTOMESSAGE);
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) io
                .sendFewArgs(sender, "/bm automessage add (msg)");
        else {
            if (has(sender, "bm.automessage.add")) {
                ...
                io.send(sender,
                        io.translate("Command.Automessage.Add").replaceAll(
                                "%index%",
                                String.valueOf(msgReader.addMessage(msg))));
            }
        }
        return true;
    }
