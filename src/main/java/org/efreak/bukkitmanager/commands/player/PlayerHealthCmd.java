...
public class PlayerHealthCmd extends Command {
    ...
    public PlayerHealthCmd() {
        super("health", "Player.Health", "bm.player.health", Arrays.asList(
                "(get|set|add|remove)", "[health]", "[player]"),
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) io.sendFewArgs(sender,
                "/bm player health (get|set|add|remove) [health] [player]");
        else if (args.length > 4) io.sendManyArgs(sender,
                "/bm player health (get|set|add|remove) [health] [player]");
        else {
            if (args[1].equalsIgnoreCase("get")) {
                if (args.length == 2 && sender instanceof Player) {
                    if (has(sender, "bm.player.health.get.your")) io.send(
                            sender,
                            io.translate("Command.Player.Health.Get.Your")
                                    .replaceAll(
                                            "%health%",
                                            String.valueOf(((Player) sender)
                                                    .getHealth())));
                    ...
                    if (has(sender, "bm.player.health.get.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Health.Get.Other")
                                            .replaceAll("%player%",
                                                    player.getName())
                                            .replaceAll(
                                                    "%health%",
                    ...
                    if (has(sender, "bm.player.health.set.your")) {
                        try {
                            new BmPlayer((OfflinePlayer) sender)
                                    .setHealth(new Integer(args[2]));
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Health.Set.Your")
                                            .replaceAll("%health%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Health.Error"));
                    ...
                    if (has(sender, "bm.player.health.set.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            try {
                                ...
                                io.send(sender,
                                        io.translate(
                                                "Command.Player.Health.Set.Other")
                                                .replaceAll("%player%",
                                                        player.getName())
                                                .replaceAll("%health%", args[2]));
                            } catch (NumberFormatException e) {
                    ...
                    if (has(sender, "bm.player.health.add.your")) {
                        ...
                        try {
                            ...
                            if (newHealth > player.getMaxHealth()) {
                                io.sendError(
                                        sender,
                                        io.translate("Command.Player.Health.TooMuch"));
                                return true;
                            }
                            player.setHealth(newHealth);
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Health.Add.Your")
                                            .replaceAll("%health%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Health.Error"));
                    ...
                    if (has(sender, "bm.player.health.add.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            try {
                                ...
                                io.send(sender,
                                        io.translate(
                                                "Command.Player.Health.Add.Other")
                                                .replaceAll("%player%",
                                                        player.getName())
                                                .replaceAll(
                                                        "%health%",
                                                        String.valueOf(newHealth)));
                            } catch (NumberFormatException e) {
                                io.sendError(
                                        sender,
                                        io.translate("Command.Player.Health.Error"));
                ...
                if (args.length == 3 && sender instanceof Player) {
                    if (has(sender, "bm.player.health.remove.your")) {
                        ...
                        try {
                            ...
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Health.Remove.Your")
                                            .replaceAll("%health%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Health.Error"));
                    ...
                    if (has(sender, "bm.player.health.remove.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            try {
                                ...
                                io.send(sender,
                                        io.translate(
                                                "Command.Player.Health.Remove.Other")
                                                .replaceAll("%player%",
                                                        player.getName())
                                                .replaceAll(
                                                        "%health%",
