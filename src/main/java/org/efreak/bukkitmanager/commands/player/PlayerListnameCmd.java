...
public class PlayerListnameCmd extends Command {
	...
	public boolean execute(CommandSender sender, String[] args) {
		if (args.length < 2) io.sendFewArgs(sender, "/bm player listname (get|set|reset) [listname] [player]");
		else if (args.length > 4) io.sendManyArgs(sender, "/bm player listname (get|set|reset) [listname] [player]");
		else {
			//get
