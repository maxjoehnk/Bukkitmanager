...
public class PlayerShowCmd extends Command {
    ...
    public PlayerShowCmd() {
        super("show", "Player.Show", "bm.player.show", Arrays
            ...
            if (args.length == 1 && sender instanceof Player) {
                if (has(sender, "bm.player.show.your")) {
                    ...
                    if (player.isVisible()) io.sendError(sender,
                            io.translate("Command.Player.Show.Already.You"));
                    else {
                        player.show();
                        io.send(sender, io.translate("Command.Player.Show.You"));
                        io.sendConsole(io.translate(
                                "Command.Player.Show.Console.You").replaceAll(
                ...
                if (has(sender, "bm.player.show.other")) {
                    ...
                    if (offPlayer != null) {
                        ...
                        if (player.isVisible()) io.sendError(sender, io
                                .translate("Command.Player.Show.Already.Other")
                                .replaceAll("%player%", player.getName()));
                        else {
                            player.show();
                            io.send(sender,
                                    io.translate("Command.Player.Show.Other")
                                            .replaceAll("%player%",
                                                    player.getName()));
                            io.send(player.getPlayer(),
                                    io.translate("Command.Player.Show.ByOther")
                                            .replaceAll("%player%",
                                                    sender.getName()));
                            io.sendConsole(io
                                    .translate(
                                            "Command.Player.Show.Console.Other")
                                    .replaceAll("%player%", player.getName())
