...
public class PlayerFirstseenCmd extends Command {
    ...
    public PlayerFirstseenCmd() {
        super("firstseen", "Player.Firstseen", "bm.player.firstseen", Arrays
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io.sendFewArgs(sender,
                "/player firstseen [player]");
        else if (args.length > 2) io.sendManyArgs(sender,
                "/player firstseen [player]");
        else {
            if (args.length == 1 && sender instanceof Player) {
                if (has(sender, "bm.player.firstseen.your")) io.send(
                        sender,
                        io.translate("Command.Player.Firstseen.Your")
                                .replaceAll(
                                        "%firstseen%",
                ...
                if (has(sender, "bm.player.firstseen.other")) {
                    ...
                    if (player != null) io.send(
                            sender,
                            io.translate("Command.Player.Firstseen.Other")
