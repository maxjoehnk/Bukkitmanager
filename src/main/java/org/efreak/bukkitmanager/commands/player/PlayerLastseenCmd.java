...
public class PlayerLastseenCmd extends Command {
    ...
    public PlayerLastseenCmd() {
        super("lastseen", "Player.Lastseen", "bm.player.lastseen", Arrays
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io.sendFewArgs(sender,
                "/bm player lastseen [player]");
        else if (args.length > 2) io.sendManyArgs(sender,
                "/bm player lastseen [player]");
        else {
            if (args.length == 1 && sender instanceof Player) {
                if (has(sender, "bm.player.lastseen.your")) io.send(
                        sender,
                        io.translate("Command.Player.Lastseen.Your")
                                .replaceAll(
                                        "%lastseen%",
                ...
                if (has(sender, "bm.player.lastseen.other")) {
                    ...
                    if (player != null) io.send(
                            sender,
                            io.translate("Command.Player.Lastseen.Other")
