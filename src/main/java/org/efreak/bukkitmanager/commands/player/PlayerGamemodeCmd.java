...
public class PlayerGamemodeCmd extends Command {
    ...
    public PlayerGamemodeCmd() {
        super("gamemode", "Player.Gamemode", "bm.player.gamemode", Arrays
                .asList("(gamemode)", "[player]"), CommandCategory.PLAYER);
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) io.sendFewArgs(sender,
                "/bm player gamemode (gamemode) [player]");
            ...
            try {
                ...
                if (GameMode.getByValue(gamemodeInt) != null) gamemode = GameMode
                        .getByValue(gamemodeInt);
                else {
                    io.sendError(sender,
                            io.translate("Command.Player.Gamemode.Unknown"));
                    return true;
                }
