...
public class PlayerLevelCmd extends Command {
    ...
    public PlayerLevelCmd() {
        super("level", "Player.Level", "bm.player.level", Arrays.asList(
                "(get|set)", "[level]", "[player]"), CommandCategory.PLAYER);
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) io.sendFewArgs(sender,
                "/bm player level (get|set) [level] [player]");
        else if (args.length > 3) io.sendManyArgs(sender,
                "/bm player level (get|set) [level] [player]");
        else {
            if (args[1].equalsIgnoreCase("get")) {
                if (args.length == 2 && sender instanceof Player) {
                    if (has(sender, "bm.player.level.get.your")) io.send(
                            sender,
                            io.translate("Command.Player.Level.Get.Your")
                                    .replaceAll(
                                            "%level%",
                    ...
                    if (has(sender, "bm.player.level.get.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Level.Get.Other")
                                            .replaceAll("%player%",
                                                    player.getName())
                                            .replaceAll(
                                                    "%level%",
                    ...
                    if (has(sender, "bm.player.level.set.your")) {
                        try {
                            new BmPlayer((OfflinePlayer) sender)
                                    .setLevel(new Integer(args[2]));
                            io.send(sender,
                                    io.translate(
                                            "Command.Player.Level.Set.Your")
                                            .replaceAll("%level%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Level.Error"));
                    ...
                    if (has(sender, "bm.player.level.set.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            try {
                                ...
                                io.send(sender,
                                        io.translate(
                                                "Command.Player.Level.Set.Other")
                                                .replaceAll("%player%",
                                                        player.getName())
                                                .replaceAll("%level%", args[2]));
                            } catch (NumberFormatException e) {
