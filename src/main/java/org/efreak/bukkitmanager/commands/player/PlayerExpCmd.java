...
public class PlayerExpCmd extends Command {
    ...
    public PlayerExpCmd() {
        super("exp", "Player.Exp", "bm.player.exp", Arrays.asList(
                "(get|set|add)", "[exp]", "[player]"), CommandCategory.PLAYER);
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) io.sendFewArgs(sender,
                "/bm player exp (get|set|add) [exp] [player]");
            ...
            if (args[1].equalsIgnoreCase("get")) {
                if (args.length == 2 && sender instanceof Player) {
                    ...
                    if (has(sender, "bm.player.exp.get.your")) io.send(
                            sender,
                            io.translate("Command.Player.Exp.Get.Your")
                                    .replaceAll("%exp%",
                    ...
                    if (has(sender, "bm.player.exp.get.other")) {
                        ...
                        if (offPlayer != null) {
                            ...
                            io.send(sender,
                                    io.translate("Command.Player.Exp.Get.Other")
                                            .replaceAll("%player%",
                                                    player.getName())
                                            .replaceAll(
                                                    "%exp%",
                ...
                if (args.length == 3 && sender instanceof Player) {
                    if (has(sender, "bm.player.exp.set.your")) {
                        try {
                            ...
                            io.send(sender,
                                    io.translate("Command.Player.Exp.Set.Your")
                                            .replaceAll("%exp%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Exp.Error"));
                    ...
                    if (has(sender, "bm.player.exp.set.other")) {
                        ...
                        if (offPlayer != null) {
                            try {
                                ...
                                io.send(sender,
                                        io.translate(
                                                "Command.Player.Exp.Set.Other")
                                                .replaceAll("%player%", args[3])
                                                .replaceAll("%exp%", args[2]));
                            } catch (NumberFormatException e) {
                                io.sendError(sender, io
                                        .translate("Command.Player.Exp.Error"));
                                if (config.getDebug()) e.printStackTrace();
                            }
                        } else io.sendError(sender,
                                io.translate("Command.Player.UnknownPlayer"));
                    ...
                    if (has(sender, "bm.player.exp.add.your")) {
                        try {
                            new BmPlayer((OfflinePlayer) sender)
                                    .giveExp(new Integer(args[2]));
                            io.send(sender,
                                    io.translate("Command.Player.Exp.Add.Your")
                                            .replaceAll("%exp%", args[2]));
                        } catch (NumberFormatException e) {
                            io.sendError(sender,
                                    io.translate("Command.Player.Exp.Error"));
