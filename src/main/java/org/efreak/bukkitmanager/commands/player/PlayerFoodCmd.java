...
public class PlayerFoodCmd extends Command {
	...
	public boolean execute(CommandSender sender, String[] args) {
		if (args.length < 2) io.sendFewArgs(sender, "/bm player food (get|set|add|remove) [food] [player]");
		else if (args.length > 4) io.sendManyArgs(sender, "/bm player food (get|set|add|remove) [food] [player]");
		else {
