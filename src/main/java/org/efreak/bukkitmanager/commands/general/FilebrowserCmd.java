...
public class FilebrowserCmd extends Command {
    ...
    public FilebrowserCmd() {
        super("filebrowser", "Filebrowser", "bm.filebrowser",
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        io.createConversation(sender, sender.getName() + ".filebrowser",
                new StartPrompt().init(sender.getName()));
        return true;
    }
...
class StartPrompt extends MessagePrompt {
    ...
    public String getPromptText(ConversationContext context) {
        ...
        conversable
                .sendRawMessage("Welcome to the Filebrowser of Bukkitmanager.");
        conversable
                .sendRawMessage("The Filebrowser is using the same commands as on the most Unix OS.");
...
class CommandQuery extends StringPrompt {
    ...
    public String getPromptText(ConversationContext context) {
        return "Current path: "
                + FilebrowserCmd.paths.get(context.getSessionData("ID"))
                        .toString();
    }
    ...
    public String getPromptText(ConversationContext context) {
        ...
        if (input.equals("cd")) return "";
            ...
            if (args.length > 1) return "";
                ...
                String arg = args[0];
                File oldDir = FilebrowserCmd.paths.get(context
                        .getSessionData("ID"));
                File newDir;
                if (arg.startsWith(File.pathSeparator)) newDir = new File(arg);
                else newDir = new File(oldDir, arg);
                if (!newDir.isDirectory()) return newDir.toString()
                        + ": is no Directory!";
                else if (!newDir.exists()) return newDir.toString()
                        + ": File or Directory not found!";
                else {
                    FilebrowserCmd.paths.put(context.getSessionData("ID")
                            .toString(), newDir);
                    return "Changed path";
                }
            }
...
class ls extends FileCommand {
    ...
    public String getPromptText(ConversationContext context) {
        String input = context.getSessionData("input").toString();
        if (input.equals("ls")) {
            File[] files = FilebrowserCmd.paths.get(
                    context.getSessionData("ID")).listFiles();
            for (int i = 0; i < files.length; i++) {
                File file = files[i];
                if (file.isDirectory()) context.getForWhom().sendRawMessage(
                        ChatColor.BLUE + file.getName());
                else context.getForWhom().sendRawMessage(
                        ChatColor.WHITE + file.getName());
            }
        } else {
            // String[] args = input.substring(input.indexOf(" ")).split(" ");
        }
