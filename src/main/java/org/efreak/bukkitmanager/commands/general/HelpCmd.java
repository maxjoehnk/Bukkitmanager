...
public class HelpCmd extends Command {
    ...
    public HelpCmd() {
        super("help", "Help", "bm.help", new ArrayList<String>(),
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        for (int i = 0; i < helpTopics.size(); i++) {
            if (helpTopics.get(i).hasPerm(sender)) topics.add(helpTopics.get(i)
                    .format());
        }
        int pages = 1;
        ...
        if (args.length == 1) {
            io.sendHeader(
                    sender,
                    config.getString("IO.HelpHeader")
                            .replaceAll("%page%", String.valueOf(1))
                            .replaceAll("%pages%", String.valueOf(pages)));
            for (int i = 0; i < 9 && i < topics.size(); i++)
                io.send(sender, topics.get(i), false);
            ...
            if (args[1].equalsIgnoreCase("caption")) {
                ...
                try {
                    if (new Integer(args[1]) <= pages) {
                        ...
                        io.send(sender,
                                config.getString("IO.HelpHeader")
                                        .replaceAll("%page%", args[1])
                                        .replaceAll("%pages%",
                                                String.valueOf(pages)), false);
                        for (int i = 9 * page - 9; i < 9 * page
                                && i < topics.size() - 1; i++)
                            io.send(sender, topics.get(i), false);
