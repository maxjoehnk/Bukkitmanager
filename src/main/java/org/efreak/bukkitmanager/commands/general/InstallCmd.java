...
public class InstallCmd extends Command {
    ...
    public InstallCmd() {
        super("install", "Install", "bm.install", new ArrayList<String>(),
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 0) io.sendFewArgs(sender, "/bm install");
            ...
            if (has(sender, "bm.install")) io.createConversation(sender,
                    "Bukkitmanager Installation", new WelcomePrompt());
        }
        return true;
    }
}
...
class PermissionsPrompt extends BooleanPrompt {
    ...
    protected Prompt acceptValidatedInput(ConversationContext context,
        ...
        if (Permissions.usePerms == true
                && PluginManager.getPlugin("Vault") != null) return new VaultPrompt();
...
class IOPrefixPrompt extends StringPrompt {
    ...
    public Prompt acceptInput(ConversationContext context, String input) {
        if (!input.equalsIgnoreCase("default")) new Configuration().set(
                "IO.Prefix", input);
