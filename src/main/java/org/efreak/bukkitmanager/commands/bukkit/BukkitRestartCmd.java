...
public class BukkitRestartCmd extends Command {
    ...
    public BukkitRestartCmd() {
        super("restart", "Bukkit.Restart", "bm.bukkit.restart",
                new ArrayList<String>(), CommandCategory.BUKKIT);
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io
                .sendFewArgs(sender, "/bm bukkit restart [time]");
        else if (args.length > 2) io.sendManyArgs(sender,
                "/bm bukkit restart [time]");
        else {
            if (has(sender, "bm.bukkit.restart")) {
                String cmd = "java -jar "
                        + Bukkitmanager.getPluginFile().toString();
                if (args.length == 1) {
                    io.broadcast(io.translate("Bukkit.Restart.Now"));
                    cmd += "--external --task restart";
                } else {
                    io.broadcast(io.translate("Bukkit.Restart.Timer")
