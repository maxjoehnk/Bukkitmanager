...
public class AutobackupIntervalCmd extends Command {
    ...
    public AutobackupIntervalCmd() {
        super("interval", "Autobackup.Interval", "bm.autobackup.interval",
                Arrays.asList("[interval]"), CommandCategory.AUTOBACKUP);
    }

    @Override
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 1) io.sendFewArgs(sender,
                "/bm autobackup interval [interval]");
        else if (args.length > 2) io.sendManyArgs(sender,
                "/bm autobackup interval [interval]");
        else {
            if (args.length == 1) {
                if (has(sender, "bm.autobackup.interval.get")) io
                        .send(sender,
                                io.translate("Command.Autobackup.Interval.Get")
                                        .replaceAll(
                                                "%interval%",
                ...
                if (has(sender, "bm.autobackup.interval.set")) {
                    io.send(sender,
                            io.translate("Command.Autobackup.Interval.Set")
                                    .replaceAll("%interval_new", args[1])
                                    .replaceAll(
                                            "%interval_old%",
                                            config.getString("Autobackup.Interval")));
                    ...
                    .sendManyArgs(sender, "/bm autobackup interval [interval]");
        }
        return true;
    }
}
