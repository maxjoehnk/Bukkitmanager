...
public class BukkitListener implements Listener {
    ...
    public void onPlayerKick(PlayerKickEvent event) {
        if (event.getPlayer().hasPermission("bm.unkickable")) event
                .setCancelled(true);
    }
        ...
        if (Permissions.has(event.getPlayer(), "bm.notification.updates")) {
            ...
            for (int i = 0; i < plugins.length; i++) {
                try {
                    ...
                    io.sendError(event.getPlayer(),
                            "Couldn't check for Plugin updates");
                    if (config.getDebug()) e.printStackTrace();
                }
            }
            if (updateCount != 0) io.send(event.getPlayer(),
                    io.translate("Command.Plugin.Update.UpdatesAvailable")
                            .replaceAll("%count%", String.valueOf(updateCount))
                            .replaceAll("%names%", names.toString()));
        }
    }
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
        if (event.getMessage().equalsIgnoreCase("/plugins")
                && config.getBoolean("Fakepluginlist.Enabled")) {
    ...
    public void chatFilter(AsyncPlayerChatEvent event) {
        if (config.getBoolean("Chatfilter.Enabled")) event
                .setMessage(ChatFilter.filter(event.getMessage()));
    }
}
