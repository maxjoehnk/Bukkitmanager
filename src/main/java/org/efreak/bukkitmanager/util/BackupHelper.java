...
public class BackupHelper {
    ...
    public void performBackup() {
        if (config.getString("Autobackup.Taskmode").equalsIgnoreCase("sync")) plugin
                .getServer().getScheduler().runTask(plugin, new Runnable() {
                    public void run() {
                        backup();
                    }
                });
        else plugin.getServer().getScheduler()
    ...
    private void backup() {
        ...
        inProgress = true;
        if (config.getBoolean("Notifications.Autobackup.Started")) NotificationsHandler
                .notify("Bukkitmanager", "Autobackup Started", "");
        if (config.getBoolean("Autobackup.Notification")) io.broadcast(io
                .translate("Autobackup.Notification.Start"));
        if (!ThreadManager.isRunning(ThreadType.AUTOSAVE)) saveHelper
                .performSave();
        try {
            ...
            for (int i = 0; i < worlds.size(); i++)
                if (config.getBoolean("Autobackup.Backup.Worlds."
                        + worlds.get(i).getName())) copyWorld(worlds.get(i));
            if (config.getBoolean("Autobackup.Backup.Plugins")) copyPlugins();
            if (config.getBoolean("Autobackup.Backup.craftbukkit")) copyJar();
            File backupFile = new File(FileHelper.getBackupDir()
                    + File.separator
                    + new Date().toGMTString().replaceAll(" ", "_")
                            .replaceAll(":", "-").replaceAll("_GMT", "")
                    + ".zip");
            ArchiveManager.compressZip(FileHelper.getTempBackupDir(),
                    backupFile);
            ...
            if (config.getBoolean("Autobackup.PostExecution.Enabled")) Runtime
                    .getRuntime()
                    .exec(new File(FileHelper.getPluginDir(), config
                            .getString("Autobackup.PostExecution.File"))
                            .toString());
            if (config.getInt("Autobackup.KeepBackups") == 0) removeOld(0);
            if (config.getBoolean("Autobackup.Notification")) io.broadcast(io
                    .translate("Autobackup.Notification.Finish"));
            if (config.getBoolean("Notifications.Autobackup.Finished")) NotificationsHandler
                    .notify("Bukkitmanager",
                            "Autobackup Finished",
                            "Size: "
                                    + Downloader.readableSize(FileUtils
                                            .sizeOf(backupFile)));
        } catch (IOException e) {
    ...
    private void copyWorld(World world) {
        try {
            FileUtils.copyDirectory(world.getWorldFolder(),
                    new File(FileHelper.getTempBackupDir() + File.separator
                            + world.getName()));
        } catch (IOException e) {
    ...
    private void copyPlugins() {
        try {
            FileUtils.copyDirectory(plugin.getDataFolder().getParentFile(),
                    new File(FileHelper.getTempBackupDir() + File.separator
                            + "plugins"));
        } catch (IOException e) {
    ...
    private void copyJar() {
        try {
            if (config.getBoolean("Autobackup.RenameJar")) FileUtils.copyFile(
                    FileHelper.getBukkitFile(),
                    new File(FileHelper.getTempBackupDir(),
                            File.separator
                                    + "craftbukkit#"
                                    + Bukkit.getVersion().split("-b")[1]
                                            .split("jnks")[0] + ".jar"));
            else FileUtils.copyFileToDirectory(FileHelper.getBukkitFile(),
                    FileHelper.getTempBackupDir());
        } catch (IOException e) {
    ...
    private void removeOld(int amount) {
        File[] files = Arrays.asList(FileHelper.getBackupDir().listFiles())
                .toArray(new File[0]);
        ...
        List<File> oldFiles;
        if (amount != 0) oldFiles = Arrays.asList(files).subList(0,
                files.length - 1 - amount);
