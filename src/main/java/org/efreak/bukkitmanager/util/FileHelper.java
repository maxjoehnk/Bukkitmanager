...
public class FileHelper {
	...
	private static File bukkitDir, pluginsDir, pluginDir, updateDir, backupDir, tempBackupDir, scriptDir, tutorialsDir;
	private static File bukkitFile, pluginFile;
	
	static {
		config = Bukkitmanager.getConfiguration();
		io = Bukkitmanager.getIOManager();
		pluginDir = Bukkitmanager.getInstance().getDataFolder().getAbsoluteFile();
	...
	public static void loadBukkitFile() {
		if (new File(bukkitDir, "craftbukkit.jar").exists()) bukkitFile = new File(bukkitDir, "craftbukkit.jar");
			...
			else io.sendConsoleError(io.translate("Autobackup.JarDoesntExists").replaceAll("%file%", config.getString("General.BukkitFile")));
		}else {
			File[] bukkitFolderFiles = bukkitDir.listFiles(new FileFilter(".jar"));
	...
	public static String parsePath(String input) {
		...
		if (backupDir != null) input = input.replaceAll("%backupdir%", backupDir.getPath());
		if (tempBackupDir != null) input = input.replaceAll("%tempbackupdir%", backupDir.getPath());
		return input;
	}
