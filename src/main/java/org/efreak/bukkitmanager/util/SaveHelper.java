...
public class SaveHelper {
    ...
    public void performSave() {
        if (config.getString("Autosave.Taskmode").equalsIgnoreCase("sync")) plugin
    ...
    private void save() {
        ...
        saveInProgress = true;
        if (config.getBoolean("Autosave.Notification")) io.broadcast(io
                .translate("Autosave.Notification.Start"));
        savePlayers();
        saveWorlds();
        if (config.getBoolean("Autosave.Notification")) io.broadcast(io
                .translate("Autosave.Notification.Finish"));
    ...
    private void saveWorlds() {
        ...
        while (itr.hasNext()) {
            ...
            io.sendConsole(io.translate("Autosave.Saving.World").replaceAll(
                    "%world%", world.getName()));
            world.save();
            i++;
        }
        io.sendConsole(io.translate("Autosave.Saved.Worlds").replaceAll(
                "%worlds%", String.valueOf(i)));
    }
}
