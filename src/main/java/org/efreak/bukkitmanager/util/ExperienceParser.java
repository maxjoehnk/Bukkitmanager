...
public class ExperienceParser {
    ...
    /**
     * Give the player some experience (possibly negative) and ensure player's
     * level and client XP bar is correctly updated too.
     * 
     * @param player
     *            The player to grant XP to
     * @param xp
     *            The amount of XP to grant
     */
    public static void awardExperience(BmPlayer player, int xp) {
        if (xp < 0 && -xp > player.getTotalExp()) return;
        ...
        if (curLevel != newLevel) {
            ...
            float neededForThisLevel = experienceNeeded(newLevel + 1)
                    - xpForThisLevel;
    ...
    /**
     * Return the level that the given player should be. (Note: the player's
     * current level might not be this, because Minecraft's experience system is
     * just plain bizarre).
     * 
     * @param player
