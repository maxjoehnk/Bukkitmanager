...
public class HelpManager {
    ...
    public static void registerCommand(Command arg1Cmd) {
        String cmd = "/bm ";
        if (arg1Cmd.getCategory().equals(CommandCategory.GENERAL)) cmd += arg1Cmd
                .getLabel();
        else cmd += arg1Cmd.getCategory().toString().toLowerCase() + " "
                + arg1Cmd.getLabel();
        String args;
        if (arg1Cmd.getArgs().size() != 0) {
            args = arg1Cmd.getArgs().get(0);
            for (int i = 1; i < arg1Cmd.getArgs().size(); i++)
                args += " " + arg1Cmd.getArgs().get(i);
        } else args = "";
        registerTopic(new HelpTopic(cmd, args, helpFile.getHelp(arg1Cmd
                .getHelpNode()), arg1Cmd.getHelpPermission()));
    }
