...
public class Configuration {
    ...
    public void updateConfig() throws IOException {
        ...
        if (update("Autosave.Taskmode", "async")
                ...
                && !getString("Autosave.Taskmode").equalsIgnoreCase("async")) set(
                "Autosave.Taskmode", "async");
        ...
        if (update("Autobackup.Taskmode", "async")
                ...
                && !getString("Autobackup.Taskmode").equalsIgnoreCase("async")) set(
                "Autobackup.Taskmode", "async");
        updateWorlds();
        ...
        update("CustomMessages.Join",
                Arrays.asList("&e%player% joined the game."));
        update("CustomMessages.Leave",
                Arrays.asList("&e%player% has left the game."));
        update("CustomMessages.Kick",
                Arrays.asList("&e%player% has left the game."));
        update("Fakepluginlist.Enabled", true);
        update("Fakepluginlist.Fakes",
                Arrays.asList("fakeplugin123", "AntiGrief"));
        update("Fakepluginlist.Hidden", Arrays.asList("*", "--BukkitManager"));
        /*
         * update("Scoreboards.Enabled", true); update("Scoreboards.Updates",
         * true); update("Scoreboards.OnlinePlayer", false);
         */
        ...
        if (!PluginManager.isPluginEnabled("Notifications")
                && getBoolean("Notifications.Enabled")) io
                .sendConsoleWarning("Notifications enabled but Plugin couldn't be found");
        if (devMode) {
        ...
        /*
         * if (devMode) { update("Swing.Enabled", false); update("Swing.Start",
         * "on-load"); update("FTPServer.Enabled", false);
         * update("FTPServer.SSL", false); update("FTPServer.Port", 21);
    ...
    private void updateWorlds() {
        ...
        for (int i = 0; i < worlds.size(); i++)
            update("Autobackup.Backup.Worlds." + worlds.get(i).getName(), true);
    }
    ...
    public void addAlias(String cmd) {
        update("General.Aliases." + cmd.substring(0, 1).toUpperCase()
                + cmd.substring(1), false);
    }

    public boolean getAlias(String cmd) {
        return getBoolean("General.Aliases."
