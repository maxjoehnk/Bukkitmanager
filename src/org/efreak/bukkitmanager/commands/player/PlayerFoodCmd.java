...
public class PlayerFoodCmd extends Command {
	...
	public boolean execute(CommandSender sender, String[] args, Integer length) {
		if (args.length < (2 + length)) io.sendFewArgs(sender, "/bm player food (get|set|add|remove) [food] [player]");
		else if (args.length > (4 + length)) io.sendManyArgs(sender, "/bm player food (get|set|add|remove) [food] [player]");
		else {
			if (args[1 + length].equalsIgnoreCase("get")) {
				if (args.length == (2 + length) && sender instanceof Player) {
					if (has(sender, "bm.player.food.get.your")) io.send(sender, io.translate("Command.Player.Food.Get.Your").replaceAll("%food%", String.valueOf(new BmPlayer((OfflinePlayer) sender).getFoodLevel())));
				}
				...
				if (args.length == (3 + length) && sender instanceof Player) {
					if (has(sender, "bm.player.food.set.your")) {
						try {
							new BmPlayer((OfflinePlayer) sender).setFoodLevel(new Integer(args[2]));
					...
					if (has(sender, "bm.player.food.set.other")) {
						...
						if (offPlayer != null) {
							BmPlayer player = new BmPlayer(offPlayer);
							try {
								player.setFoodLevel(new Integer(args[2 + length]));
								io.send(sender, io.translate("Command.Player.Food.Set.Other").replaceAll("%player%", player.getName()).replaceAll("%food%", args[2 + length]));
							}catch (NumberFormatException e) {
								io.sendError(sender, io.translate("Command.Player.Food.Error"));
								if (config.getDebug()) e.printStackTrace();
							}
				...
				if (args.length == (3 + length) && sender instanceof Player) {
					if (has(sender, "bm.player.food.add.your")) {
						...
						try {
							int newFood = player.getFoodLevel() + new Integer(args[2 + length]);
							if (newFood > player.getMaxHealth()) {
								io.sendError(sender, io.translate("Command.Player.Food.TooMuch"));
								return true;
							}
							player.setFoodLevel(newFood);
							io.send(sender, io.translate("Command.Player.Food.Add.Your").replaceAll("%food%", args[2 + length]));
						}catch (NumberFormatException e) {
							io.sendError(sender, io.translate("Command.Player.Food.Error"));
							if (config.getDebug()) e.printStackTrace();
						}
				...
				if (args.length == 3 && sender instanceof Player) {
					if (has(sender, "bm.player.food.remove.your")) {
						...
						try {
							...
							player.setFoodLevel(newFood);
							io.send(sender, io.translate("Command.Player.Food.Remove.Your").replaceAll("%food%", args[2 + length]));
						}catch (NumberFormatException e) {
							io.sendError(sender, io.translate("Command.Player.Food.Error"));
							if (config.getDebug()) e.printStackTrace();
						}
