...
public class BmPlugin {
	...
	public static void player(Player p, String[] args)
	{
		...
		else if (args[1].equalsIgnoreCase("download")) if (hasPerm(p, "bm.plugin.download")) BmPluginDownload.player(p, args);
	...
	public static void console(ConsoleCommandSender c, String[] args)	{
		if (!(args.length == 1)) {
			...
			else if (args[1].equalsIgnoreCase("download")) BmPluginDownload.console(c, args);
